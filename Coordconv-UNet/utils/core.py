    ...
    for idx, (img, mask) in enumerate(dataset_val):
        ...
        with torch.no_grad():
            ...
            for iw in range(len(img.cpu().numpy())):
                ...
                #pred=pred*255
                #true = true*255
                # cv2.imwrite('/home/vfuser/sungjoo/Resize_model/res/image/'+str(k)+'.jpg',origin)
                # cv2.imwrite('/home/vfuser/sungjoo/Resize_model/res/pred/'+str(k)+'.png',pred)
...
def euclidean_dis(y_pred,y_true):
    return torch.sqrt(torch.sum(torch.square(y_pred-y_true),axis=-1))

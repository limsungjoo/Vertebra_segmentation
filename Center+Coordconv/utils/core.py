...
def train(net, dataset_trn, optimizer, criterion, epoch, opt,train_writer):
    ...
    for it, (img, mask, filename) in enumerate(dataset_trn):
...
def validate(dataset_val, net, criterion, epoch, opt, best_iou, best_epoch,train_writer):
    ...
    for it, (img, mask,filename) in enumerate(dataset_val):
        # Load Data
        img, mask = torch.Tensor(img).float(), torch.Tensor(mask).float()
        if opt.use_gpu:
            img, mask = img.cuda(non_blocking=True), mask.cuda(non_blocking=True)
...
def evaluate(dataset_val, net, opt):
    ...
    i=0
    k=0
    for idx, (img, mask,file_name) in tqdm(enumerate(dataset_val)):
        # Load Data
        # print(file_name[0])
        img = torch.Tensor(img).float()
        if opt.use_gpu:
        ...
        with torch.no_grad():
            ...
            for iw in range(len(img.cpu().numpy())):
                ...
                # cv2.imwrite('/home/vfuser/sungjoo/Center+Coordconv/external_val/spine/'+str(k)+'.png',origin)
                cv2.imwrite('/home/vfuser/sungjoo/Center+Coordconv/external_val/GE_fake_pred/'+str(k)+'.png',pred)
                cv2.imwrite('/home/vfuser/sungjoo/Center+Coordconv/external_val/gt/'+str(k)+'.png',true)
                ...
                #                 pred = cv2.bitwise_and(pred,morph)
                                
                # #     cv2.imwrite('/home/vfuser/sungjoo/Center+Coordconv/exp/contour2pred/'+str(k)+'.png',pred)
                #     dice = compute_dice_coefficient(true, pred)
                # #     pre, rec ,acc, spe= compute_pre_rec(true,pred)
                #     i+=dice
                #     k+=1
                    # cv2.imwrite('/home/vfuser/sungjoo/Center+Coordconv/exp/final_res/true/'+str(k)+'.png',true)
                    # cv2.imwrite('/home/vfuser/sungjoo/Center+Coordconv/exp/final_res/pred/'+str(k)+'.png',pred)
                    # cv2.imwrite('/home/vfuser/sungjoo/Center+Coordconv/exp/final_res/origin/'+str(k)+'.jpg',origin)
                    # pre, rec ,acc, spe= compute_pre_rec(true,pred)
                #     i+=acc
                #     print(acc)
                #     # i += accuracy_score(pred,true)
                #     # i+=recall_score(pred,true)
                #     # print(i)
                #     # i+= hd95(pred,true)
                #     # i+=obj_asd(pred,true)
                    # i+=rec

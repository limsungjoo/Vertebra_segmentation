...
class VertebralDataset(Dataset):
    def __init__(self, opt, is_Train=True, augmentation=True):
        ...
        if is_Train:
            ...
            self.mask_list = self.mask_list[:int(len(self.mask_list)*0.90)]
            # print(self.mask_list)
    ...
    def __getitem__(self, index):
        ...
        xray_path = mask_path.replace('/sam_mask/', '/sam/').replace('.png', '.jpg')
        center_path = mask_path.replace('/sam_mask/', '/sam_center/').replace('.png', '.png')
        file_name = xray_path.split('/')[-1]

        img = cv2.imread(xray_path, 0)
        mask = cv2.imread(mask_path, 0)
        center = cv2.imread(center_path, 0)
        ...
        cen_resize = cv2.resize(center, (x,y))

        xs = int((512 - x)/2)
        ys = int((1024-y)/2)
        bg_img[ys:ys+y,xs:xs+x]=img_resize
        bg_msk[ys:ys+y,xs:xs+x]=msk_resize
        bg_cen[ys:ys+y,xs:xs+x]=cen_resize

        img = bg_img
        mask = bg_msk
        center = bg_cen
        img = np.stack((img,img))
        mask = np.stack((mask,center))
